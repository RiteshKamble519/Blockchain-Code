...
class Blockchain:
    ...
    def is_chain_valid(self,chain):
        ...
        while block_index < len(chain):
            ...
            proof = block['proof'] #proof = proof In Course 
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Ritesh', amount = 3)
    block = blockchain.create_block(proof,previous_hash)
    response = { 'message':'Congratulations, you just mined a Block!',
                 'index': block['index'],
                 'timestamp': block['timestamp'],
                 'proof': block['proof'],
                 'pevious_hash': block['previous_hash'],
...
def add_transaction():
      ...
      if not all (key in json for key in transaction_keys):
          return 'Some elements of the transacion are missing' , 400
      index = blockchain.add_transacion(json['sender'],json['receiver'], json['amount'])
